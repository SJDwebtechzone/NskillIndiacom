...
// Clean up description text from markdown-style content
...
// ─── Course Card ──────────────────────────────────────────────────────────────
function CourseCard({ course, index, category }: { course: any; index: number; category: string }) {
  const imgSrc = getCourseImage(category, index);
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-6">
        ...
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{course.eligibility}</span>
        </div>
        ...
        <p className="text-sm text-slate-500 leading-relaxed line-clamp-2 mb-5 flex-1">
          {desc || course.content[0]?.substring(0, 120).replace(/[#\-]/g, "").trim()}
        </p>
        ...
        <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <Award className="w-4 h-4 text-amber-500" />
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Certified Program</span>
          </div>
          <Link
            href={`/courses/${course.id}`}
            ...
            return (
              <div key={category}>
                ...
                <div className="flex items-center gap-6 mb-8">
                  <div>
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight">{category} Courses</h2>
                    <p className="text-sm text-slate-400 font-medium mt-0.5">{categoryCourses.length} programs available</p>
                  </div>
