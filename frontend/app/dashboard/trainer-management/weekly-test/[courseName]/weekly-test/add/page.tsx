...
export default function TrainerAddPostQuestionPage() {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-800 mb-1">Add Weekly-Test Question</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
        <div>
          ...
          <textarea required rows={3} value={form.question} onChange={e => setForm({...form, question: e.target.value})} placeholder="Enter your question..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-400 resize-none text-gray-800" />
        </div>
        {(['a','b','c','d'] as const).map(opt => (
          <div key={opt}>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Option {opt.toUpperCase()} *</label>
            <input required type="text" value={form[`option_${opt}` as keyof typeof form]} onChange={e => setForm({...form, [`option_${opt}`]: e.target.value})} placeholder={`Option ${opt.toUpperCase()}`} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-400 text-gray-800" />
          </div>
        ))}
        <div>
