...
export default function TrainerPretestConfigPage() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (data.config) {
        setForm({
          total_qs: data.config.total_qs,
          pass_percent: data.config.pass_percent,
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-800 mb-1">Pretest Config</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
        <div>
          ...
          <input
            ...
            min={1}
            value={form.total_qs}
            onChange={e => setForm({ ...form, total_qs: parseInt(e.target.value) })}
        ...
        <div>
          ...
          <input
            ...
            onChange={e => setForm({ ...form, pass_percent: parseInt(e.target.value) })}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-400 text-gray-800"
          />
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            = {Math.round(form.time_limit / 60)} minutes
          </p>
        </div>
