...
export default function DownloadA4Page() {
  ...
  <tr>
    ...
    <th className="text-left px-4 py-3.5 text-xs font-black uppercase tracking-wider text-slate-400">Signed Copy</th>
              ...
              <tbody>
                {filtered.map((s, i) => (
                  <tr key={s.id} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/40"}>
                    ...
                    <td className="px-4 py-3.5">
                      {s.signed_admission_file ? (
                        <button
                          onClick={() => viewSignedDocument(s)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold transition border border-emerald-200"
                        >
