...
export default function PretestQuestionsPage() {
  const { courseName } = useParams(); // ← was moduleId
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3">
          <Link
            href={`/dashboard/student-management/pretest/${courseName}/pre-test/config`}
            className="px-4 py-2 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 text-sm font-medium"
          >
            ⚙️ Config
          </Link>
          <Link
            href={`/dashboard/student-management/pretest/${courseName}/pre-test/add`}
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex justify-center gap-2">
                      <button
                        onClick={() =>
                          router.push(
                            `/dashboard/student-management/pretest/${courseName}/pre-test/edit/${q.id}`
                          )
                        }
